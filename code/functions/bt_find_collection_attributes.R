bt_find_collection_attributes=function(new.collection.name = NULL, collection.id = NULL, hints.id = NULL, starred.hint.id = NULL, country = NULL,
  ...
  pull.intervention.attributes = sprintf(paste0("SELECT gta_ids.hint_id, b221_hint_assessment.assessment_id, b221_hint_intervention.apparent_intervention_id AS intervention_id, 
                                                      b221_hint_product_group.product_group_id, bt_hint_jurisdiction.jurisdiction_id, bt_hint_relevance.relevance, bt_hint_date.`date`, bt_hint_date.date_type_id FROM 
  ...
  return(list(collection.unchanged = !collectionChanged, starred.hint.id = starred.hint.id, country = country, product = product, intervention = intervention, assessment = assessment, relevance = relevance, announcement.date = announcement.date, implementation.date = implementation.date, removal.date = removal.date))
