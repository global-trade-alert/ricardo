...
get_single_hints_infos <- function(user.id = NULL) {
  output = gta_sql_get_value(sqlInterpolate(pool, paste0("SELECT * FROM
                                                          ...
                                                          JOIN bt_hint_url ht_url ON ht_url.hint_id = ht_log.hint_id AND ht_log.hint_state_id BETWEEN 2 AND 14 JOIN bt_url_log ON ht_url.url_id = bt_url_log.url_id JOIN bt_url_type_list ON bt_url_type_list.url_type_id = ht_url.url_type_id
