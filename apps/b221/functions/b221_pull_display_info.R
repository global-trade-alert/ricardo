b221_pull_display_info = function(is.freelancer = NULL, user.id = NULL){
  ...
  if(is.freelancer == T){
    ...
    pull.display = paste0("SELECT jur_list.jurisdiction_name, ht_log.acting_agency,  ht_txt.hint_title, ht_txt.hint_description, bt_url_log.url, attributed_hints.hint_id,
                          ...
                          JOIN bt_hint_url ON bt_hint_url.hint_id = attributed_hints.hint_id AND (bt_hint_url.url_accepted <> 0 OR bt_hint_url.url_accepted IS NULL)
    ...
    pull.display = paste0("SELECT jur_list.jurisdiction_name, ht_log.acting_agency, ht_txt.hint_title, ht_txt.hint_description, bt_url_log.url, attributed_hints.hint_id, 
                            ...
                            JOIN bt_hint_url ON bt_hint_url.hint_id = attributed_hints.hint_id AND (bt_hint_url.url_accepted <> 0 OR bt_hint_url.url_accepted IS NULL) AND bt_hint_url.classification_id IS NOT NULL
                            JOIN bt_url_log ON bt_url_log.url_id = bt_hint_url.url_id
                            JOIN bt_url_type_list ON bt_hint_url.url_type_id = bt_url_type_list.url_type_id
                            LEFT JOIN bt_hint_text ht_txt ON ht_log.hint_id = ht_txt.hint_id
                            LEFT JOIN bt_hint_jurisdiction ht_jur ON ht_jur.hint_id = ht_log.hint_id LEFT JOIN gta_jurisdiction_list jur_list ON jur_list.jurisdiction_id = ht_jur.jurisdiction_id
