b221_process_collections_hints=function(is.freelancer = NULL, user.id = NULL, new.collection.name = NULL, collection.id = NULL, hints.id = NULL, country = NULL,
  ...
  if(!any(is.na(hints.id))){
    gta_sql_get_value(sprintf(paste0("DELETE FROM b221_hint_collection WHERE b221_hint_collection.collection_id = ",collection.id," AND b221_hint_collection.hint_id NOT IN (%s);"),paste(hints.id, collapse = ',')))
  ...
  if(is.null(new.collection.name)){
    if(is.freelancer == T){
      update.collection.hints  = paste0(" /* FREELANCER UPLOAD */
      ...
      update.collection.hints  = paste0("/* EDITOR UPLOAD */
                                        SET @classification_id = (SELECT AUTO_INCREMENT FROM information_schema.tables WHERE table_name='bt_classification_log');
                                        INSERT INTO bt_classification_log(classification_id, user_id, hint_state_id, time_stamp)
