b221_process_display_info=function(is.freelancer = NULL, user.id = NULL, processed.rows = NULL){
  ...
  setnames(processed.rows, c('id','clicked','country','product','intervention','assessment','url','official','comment'),
           c('hint.id','relevance','implementer.name','product.group.name','intervention.type.name','assessment.name','url','is.official','comment'))
  
  input.col.names = c('hint.id','implementer.name','url','is.official','assessment.name',
                      'product.group.name','intervention.type.name','comment','relevance')
  ...
  if(is.freelancer==T){
    push.updates = paste0("/* FREELANCER UPLOAD */
                          ...
                          UPDATE bt_hint_log
                          JOIN (SELECT DISTINCT b221_temp_changes_data_",user.id,".hint_id, relevance FROM b221_temp_changes_data_",user.id,") changes ON changes.hint_id = bt_hint_log.hint_id
