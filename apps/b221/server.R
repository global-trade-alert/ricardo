...
b221server <- function(input, output, session, user, app, prm, ...) {
  ...
  observeEvent(input$saveNewCollection, {
    ...
    if (any(nchar(colName)==0,
      ...
      if (colState == "newCollection") {
        ...
        print("collection changed val: ",collectionChanged,
              "collection impl val: ",colImplementer,
              "collection inttype val: ",colType,
              "collection assessment val: ",colAssessment,
              "collection product val: ",colProduct)
  ...
  collections <- eventReactive(input$loadCollections, {
    ...
    collectionsOutput$intervention.type.name <- gsub("export barrier","EB",collectionsOutput$intervention.type.name)
    collectionsOutput$intervention.type.name <- gsub("uncertain","UN",collectionsOutput$intervention.type.name)
    ...
    collectionsOutput$tag_type = apply(collectionsOutput,1, function(x){
      as.character(paste0("<div class='grid-row'>",paste0("<div class='tag type'>",substr(strsplit(x['intervention.type.name'],split=" ; ")[[1]],1,20),ifelse(nchar(x['intervention.type.name'])>20,"...",""),"</div>",collapse=""),"</div>"))
    })
  ...
  observeEvent(input$singleHintsTable_rows_selected, { 
    ...
    if (moveHint$hint.state.id %in% c(2,8)) {
      ...
      if (lockHint()) {
        ...
        if (length(c(
          setdiff(colImplementer, initialJurisdictions),
          setdiff(colType, initialType),
          setdiff(colAssessment, initialAssessment),
          setdiff(colProduct,initialProduct )
        )) > 0 ) {
