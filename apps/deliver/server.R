...
deliverserver <- function(input, output, session, user, app, prm, ...) {
  ...
  observeEvent(input$saveXlsx, {
    ...
    output <- 
      dlvr_pull_display(last.deliverable = paste0(lubridate::floor_date(lubridate::now(), "second")))
  ...
  observeEvent(input$confirmHint, {
    ...
    confirmedHint_classifications <- names() %>%
      ...
      select(c('hint.id', 'url.classification', 'text.classification', 'jur.classification', 'rlvnt.classification', 
               'ass.classification', 'int.classification', 'prod.classification', 'date.classification', 'discard.classification'))
